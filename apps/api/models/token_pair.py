...
class TokenPair(Base):
    ...
    pair_address = Column(String(50), nullable=False, index=True)
    ...
    base_token_id = Column(BigInteger, ForeignKey('tokens.id'), nullable=False, index=True)   # Token ID
    quote_token_id = Column(BigInteger, ForeignKey('tokens.id'), nullable=False, index=True)    # Token ID
