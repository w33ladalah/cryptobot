...
@auth_required
async def get_current_user(db: Session = Depends(get_db), user_data: dict = Depends()):
    try:
        user_service = UserRepository(db)
        return user_service.read_user(user_data.get("sub"))
    except HTTPException as e:
        return JSONResponse(status_code=e.status_code, content={"status": "error", "message": str(e)})

@router.get("/", response_model=UsersResponse, status_code=status.HTTP_200_OK)
@auth_required
async def read_users(page: int = 1, limit: int = 10, db: Session = Depends(get_db)):
    try:
        user_service = UserRepository(db)
