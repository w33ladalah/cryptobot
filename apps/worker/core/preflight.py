...
def check_sepolia_env() -> list[str]:
    ...
    if config.UNISWAP_ROUTER_ADDRESS is None:
        ...
        # Check for mainnet address first (case-insensitive) before checksum validation
        if router_address.lower() == _UNISWAP_V2_ROUTER_MAINNET.lower():
        ...
        elif not is_checksum_address(router_address):
